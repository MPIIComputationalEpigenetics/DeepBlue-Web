...
<section id="widget-grid" class="">
	...
	<div>
		<div class="input-group">
			<input id="set_name_input" class="form-control" type="text" placeholder="Experiment set name" />
			<input id="description" class="form-control" type="text" placeholder="Experiment set description"/>
		</div>
	</div>
...
<script type="text/javascript">
    ...
    var pagefunction = function() {
        ...
        $('#downloadBtnBottom').click(function(e){
            ...
            var experiment_names = selectedData.map(function (exp) {
            	return exp[1];
            });

            debugger;

			var set_name_input = $('#set_name_input').val();
			var description = $('#description').val();

            var request = $.ajax({
                url: "ajax/server_side/create_experiments_set.php",
                type: "POST",
                dataType: "json",
                data : {
            ...
            request.done( function(data) {
                ...
                swal({
                    title: "Experiment set created.",
                    text: "Your experiment set " + set_name_input + " (ID " + data[1] + ") was created. Would you like to go to this set page?",
                    type: "success",
                ...
                function(yes){
                    window.history.go(-1);
                    if (yes) {
                        window.open("/experiment_set.php?_id=" + data[1], '_blank')
                    }
                });
            });
        });
